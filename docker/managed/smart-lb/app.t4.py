...
app = FastAPI(title="Smart Load Balancer - T4 Optimized")
...
class EndpointSelector:
    ...
    async def get_best_endpoint(self, model: Optional[str] = None) -> str:
        ...
        """
        if model:
            # Route to specific model endpoint if requested (4 models for T4)
            model_map = {
...
async def list_models():
    """List all available models across endpoints (4 models for T4)."""
    models = [
